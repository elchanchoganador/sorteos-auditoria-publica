...
async function main() {
    ...
    const qrngSegmento = qrngHex ?? 'QRNG_NO_DISPONIBLE';
    ...
    if (qrngHex) {
        ...
        warn('QRNG no estuvo disponible en esta ejecución; se verificará usando el marcador QRNG_NO_DISPONIBLE.');
    }

    const nistHex = resultado.entropia?.nist?.hex ?? null;
    const nistSegmento = nistHex ?? 'NIST_NO_DISPONIBLE';
    if (nistHex && resultado.entropia?.nist?.pulse?.pulseIndex) {
        try {
            const pulse = await fetchNistPulse(resultado.entropia.nist);
            if (String(pulse.pulseIndex) === String(resultado.entropia.nist.pulse.pulseIndex)) {
                ok(`NIST pulseIndex coincide: ${pulse.pulseIndex}`);
                pasaron++;
            } else {
                fail(`NIST pulseIndex NO coincide`);
                fallaron++;
            }

            if (String(pulse.outputValue).toLowerCase() === String(nistHex).toLowerCase()) {
                ok(`NIST outputValue coincide`);
                pasaron++;
            } else {
                fail(`NIST outputValue NO coincide`);
                fallaron++;
            }
        } catch (e) {
            fail(`No se pudo verificar NIST Beacon: ${e.message}`);
            fallaron++;
        }
    } else {
        warn('NIST Beacon no estuvo disponible en esta ejecución; se verificará usando el marcador NIST_NO_DISPONIBLE.');
    }

    const semillaCalc = createHash('sha256')
        .update(`${randomness}|${hashPart}|${hashClimaCalc}|${fib.hex}|${resultado.drand.ronda}|${resultado.entropia.contexto_sorteo}|${qrngSegmento}|${nistSegmento}`, 'utf8')
        .digest('hex');
