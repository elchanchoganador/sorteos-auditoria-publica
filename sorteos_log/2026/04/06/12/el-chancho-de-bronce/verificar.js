...
async function encontrarArchivoCadena(startDir) {
    ...
    for (let i = 0; i < 8; i++) {
        const candidateNdjson = path.join(current, 'sorteos_log', 'sorteos_chain.ndjson');
        try {
            const raw = await readFile(candidateNdjson, 'utf8');
            if (raw.trim().length > 0) {
                return candidateNdjson;
            }
        } catch {
            // continuar al padre
        }

        const legacyCandidate = path.join(current, 'sorteos.json');
        try {
            const rawLegacy = await readFile(legacyCandidate, 'utf8');
            // Compat: solo acepta legacy si parece NDJSON con entry_type
            if (rawLegacy.includes('"entry_type"')) {
...
async function main() {
    ...
    if (algoritmoVersion === '3.0.0' || algoritmoVersion === '3.1.0') {
        ...
        if (!cadenaPath) {
            fail('No se encontró sorteos_chain.ndjson para verificar eventos de cadena');
            fallaron++;
        } else {
